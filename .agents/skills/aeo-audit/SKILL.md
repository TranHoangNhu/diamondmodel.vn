---
name: aeo-audit
description: "Audit a website's Answer Engine Optimization (AEO) readiness. Checks llms.txt, structured data, content answerability, and AI crawler access. Use when asked to 'audit AEO', 'check AI readiness', 'review llms.txt', 'is my site AI-friendly', or 'optimize for AI search'. Triggers: 'aeo audit', 'ai audit', 'llms.txt audit', 'ai search readiness', 'answer engine audit', 'check ai crawlers'."
version: 1.2.2
---
...
1. Run:
   ```bash
   npx @ainyc/aeo-audit@latest "<url>" --format json
   ```
2. Return:
...
1. Run:
   ```bash
   npx @ainyc/aeo-audit@latest "<url>" --format json --factors ai-readable-content
   ```
